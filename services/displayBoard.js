...
//   },
  ...
  getDisplayBoard : async (selected)=>{
    console.log("get all Displayboards", selected)
    try {
      const payload = {
        "agencyIds": selected
      }
      const res = await axiosClient.post("http://103.79.34.50:8083/api/ManageMaster/getalldisplayboards", payload);
      return res.data;
    } catch (err) {
      throw err; // interceptor will format it
    }
